...
{	
	/**
		Variables in this map will be set to every existing Brew's. 
	**/
	public static var globalVariables:GlobalBrewMap = new GlobalBrewMap();
	
	/**
		Every created BrewScript will be mapped to this map. 
	**/
	public static var global(default, null):Map<String, BrewScript> = [];
	...
	**/
	...
	/**
		The script to execute. Gets set automatically when you create a `new` Brew.
	**/
	...
	**/
	...
	/**
		...
		@param Preset If true, BrewScript will set some useful variables to interp. Override `preset` to customize the settings.
	...
	**/
	public function set(key:String, ?obj:Dynamic):BrewScript
	{
		if (_destroyed)
			return null;
	...
	/**
		A special object is the object that'll get checked if a variable is not found in a Brew.
		...
		@return Returns this instance for chaining.
	**/
	public function setSpecialObject(obj:Dynamic, ?includeFunctions:Bool = true, ?exclusions:Array<String>):BrewScript
	{
		if (_destroyed)
			return null;
		if (!active)
	...
	**/
	public function setClass(cl:Dynamic):BrewScript
	{
		if (_destroyed)
			return null;
	...
	/**
		...
		@return this instance for chaining.
	**/
	public function setClassString(cl:String):BrewScript
	{
		if (_destroyed)
			return null;
	...
	/**
		...
		@return Returns this instance for chaining.
	**/
	public function unset(key:String):BrewScript
	{
		if (_destroyed)
			return null;
	...
	**/
	public function call(func:String, ?args:Array<Dynamic>):BrewCall
	{
		if (_destroyed)
			return {
				exceptions: [new Exception((if (scriptFile != null && scriptFile.length > 0) scriptFile else "BrewScript instance") + " is destroyed.")],
				calledFunction: func,
				succeeded: false,
				returnValue: null
			};
		...
		if (!active)
			return {
				exceptions: [new Exception((if (scriptFile != null && scriptFile.length > 0) scriptFile else "BrewScript instance") + " is not active.")],
				calledFunction: func,
				succeeded: false,
				returnValue: null
			};
		...
		else if (interp == null || !exists(func))
		{
			if (scriptFile != null && scriptFile.length > 1)
				...
				pushException('Function $func does not exist in BrewScript instance.');
		}
		else 
		{
	...
	/**
		...
		@return Returns this instance for chaining.
	**/
	public function clear():BrewScript
	{
		if (_destroyed)
			return null;
		if (!active)
	...
	**/
	public function doScript(string:String #if hscriptPos, ?origin:String #end):BrewScript
	{
		if (_destroyed)
			return null;
		if (!active)
