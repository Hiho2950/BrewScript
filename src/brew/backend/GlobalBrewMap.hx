...
**/
typedef GlobalBrewMap = TypedGlobalBrewMap<String, Dynamic>;

@:transitive
@:multiType(@:followWithAbstracts K)
@:access(brew.BrewScript)
abstract TypedGlobalBrewMap<K, V>(IMap<K, V>) 
{
	public function new();
	...
	@:from static inline function fromStringMap<V>(map:StringMap<V>):TypedGlobalBrewMap<String, V> 
		return cast map;

	@:from static inline function fromIntMap<V>(map:IntMap<V>):TypedGlobalBrewMap<Int, V> 
		return cast map;

	@:from static inline function fromObjectMap<K:{}, V>(map:ObjectMap<K, V>):TypedGlobalBrewMap<K, V> 
		return cast map;
}
